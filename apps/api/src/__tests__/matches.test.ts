...
async function addSanitizedCandidate(name: string, roleCategory: string | null = null) {
...
describe('GET /matches', () => {
  ...
  it('weighs required-skill coverage over raw embedding similarity', async () => {
    ...
    // skillCoverage = 1/2, semanticSimilarity = 0.8
    // score = 0.6 * 0.5 + 0.4 * 0.8 = 0.62
  ...
  it('credits a curated skill relation when there is no exact match', async () => {
    ...
    // at 0 -> skillCoverage = 0.7/2 = 0.35; score = 0.6*0.35 + 0.4*0.8 = 0.53
    ...
    expect(match.similarity).toBeCloseTo(0.53);
  });
